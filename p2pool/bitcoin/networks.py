...
nets = dict(
    ...
    continuumcoin=math.Object(
        P2P_PREFIX='02030405'.decode('hex'), #messagestart
        P2P_PORT=25535,
        ADDRESS_VERSION=28, #pubkey_address
        RPC_PORT=25536,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'continuumcoinaddress' in (yield bitcoind.rpc_help()) and
            not (yield bitcoind.rpc_getinfo())['testnet']
        )),
        SUBSIDY_FUNC=lambda height: 1*100000000 if height<121  else 524288*100000000 >> (height * 1)//86400,
        POW_FUNC=data.hash256,
        BLOCK_PERIOD=30, # s
        SYMBOL='CTM',
        ...
        BLOCK_EXPLORER_URL_PREFIX='http://singleblade.info/ctmcoin/block_crawler.php?block_hash=',
        ADDRESS_EXPLORER_URL_PREFIX='',
        TX_EXPLORER_URL_PREFIX='http://singleblade.info/ctmcoin/block_crawler.php?transaction=',
        SANE_TARGET_RANGE=(2**256//2**32//1000 - 1, 2**256//2**32 - 1),
        DUMB_SCRYPT_DIFF=1,
        DUST_THRESHOLD=1e8,
    ),
